<img src="https://github.com/FixCrap/FixCrap/blob/main/github_prof.png?raw=true" alt="George Andriopoulos GitHub Banner" width="100%" />

# Hi, I'm George Andriopoulos 👋  
**📍 Zakynthos, Greece**  
**💻 Frontend / Full Stack Engineer**  
**🚀 Building production-grade web & mobile applications**

📫 **Reach out:**  
[![Email](https://img.shields.io/badge/-george_andriopoulos@hotmail.gr-D14836?style=flat&logo=gmail&logoColor=white)](mailto:george_andriopoulos@hotmail.gr)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/george-andriopoulos-3107372b0)
[![Portfolio](https://img.shields.io/badge/Portfolio-%23000000.svg?style=flat&logo=github&logoColor=white)](https://www.george-andriopoulos.com)

---

## 👨‍💻 About Me

I’m a **Frontend / Full Stack Engineer** focused on building **real-world, production-grade web and mobile applications** used by real users.

I specialize in **React, React Native, Next.js, and TypeScript**, working across complex domains like **AI-powered products, healthcare, and SaaS platforms**. My work goes beyond UI — I regularly make architectural decisions, performance tradeoffs, and product-level improvements.

I care deeply about **code quality, scalability, performance, and user experience**, and I enjoy owning features end-to-end — from early design discussions to production rollout and iteration.

When I’m not coding, you’ll probably find me lifting heavy weights 🏋️‍♂️, exploring new places 🌍, or refining side projects and tooling.

---

## 🌟 What I’m Up To

- 🔭 **Currently working on:** Production web & mobile apps with React, React Native, and Next.js  
- 🌱 **Currently learning:** Deeper backend & system design concepts (Node.js, Supabase, infra patterns)  
- 👯 **Open to:** Impactful product-driven roles and collaborations  
- 💬 **Ask me about:** Frontend architecture, performance optimization, UI/UX systems, or scaling real apps

---

## 🛠️ Tech Stack

![Tech Stack](https://skillicons.dev/icons?i=js,ts,react,next,nodejs,tailwind,mongodb,supabase,graphql,git,github,vercel,figma,html,css,docker)

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=George-Andriopoulos&show_icons=true&theme=tokyonight&hide_border=true" width="48%" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=George-Andriopoulos&theme=tokyonight&hide_border=true" width="48%" />
</p>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=George-Andriopoulos&theme=tokyo-night&hide_border=true" />

---

## 🌟 Most Used Languages

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=George-Andriopoulos&theme=tokyonight&layout=compact&hide_border=true" />

---

## 🌍 Let’s Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/george-andriopoulos-3107372b0)
[![Portfolio](https://img.shields.io/badge/🌐-Portfolio-2CA5E0?style=for-the-badge)](https://www.george-andriopoulos.com)
